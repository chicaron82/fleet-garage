// THE KEY-TAG READ, as a function — so the HTTP endpoint and Effie's overflow tool are the SAME
// reader.
//
// ⭐⭐ WHY THIS FILE EXISTS. Aaron, 2026-09-08, on logging a stack of tags through the chat and
// finding the record still empty: *"anything that reads keytags shouldn't be tossing out valuable
// info"*. The fix needed the chat path to read a tag properly — and the obvious shortcut was to let
// the chat model transcribe the tag itself, since it already sees the photo. He rejected that:
// *"b properly. season it right take your time with the cook."*
//
// ⭐⭐⭐ HE WAS RIGHT, AND THE REASON IS IN THIS FILE'S OWN NUMBERS. The read is not "ask a model
// what the tag says" — it is a two-tier escalation measured on 40 real tags, a fleet-corroboration
// check that decides whether to escalate, a spend ledger, the curated codex, the taught codex, and
// rental-class learn/infer with a human pin outranking the tag. A second reader would have had none
// of it while writing the same table.
//
// ⚠️ It lived inside the HTTP handler, which is why it could not be reused and why
// `api/keytag-read.ts` had grown to 378 lines against a 330 cap. Extracted 2026-09-09: the endpoint
// keeps auth, parsing and status codes; everything that IS the read lives here.
...
- COLOUR: printed tags use the FIRST THREE LETTERS of the colour word — WHI→White, BLA→Black, GRA→Gray, GRE→Green, SIL→Silver, BLU→Blue, RED→Red, BRO→Brown, ORA→Orange, PLU = Blue (it is Kia's "Pluton Blue" — the code is the first three letters of the MANUFACTURER'S name for the colour, which is why it is not Plum). ⚠️ An unlisted 3-letter code: report it AS-IS, unexpanded — do NOT guess a colour from the letters. PLU is the proof: expanding it "by the rule" gives Plum and the car is blue. Handwritten tags write a plain word ("Blue"). GRA/GRE and BLA/BLU differ by a single character, so READ the code — never infer a colour from the car, and never from the KEYS or FOB in the photo (an orange fob beside a tag printed BLA was read as Orange). Report the colour name.
...
/** A parsed data-URL image, as `parseImageDataUrl` returns it. */
export interface KeytagImage { mediaType: string; data: string }

/**
 * Read one key-tag photo, all the way through: two-tier model, fleet corroboration, the codices,
 * and rental-class learn/infer. Returns null when nothing usable came back.
 *
 * ⚠️ It WRITES two things and neither is the fleet: the spend ledger, and a learned
 * code→rental-class row. Both are deliberate and both are best-effort — the read must never fail
 * because bookkeeping did. Nothing here touches `vehicles`; deciding what a read means for a
 * vehicle record is the caller's job, through the shipped deciders.
 */
export async function readKeytagPhoto(
  image: KeytagImage,
  supabase: SupabaseClient,
  userId: string,
  apiKey: string,
): Promise<KeytagRead | null> {
  ...
  // being asked. If the fallback also fails, the original error is re-thrown so the caller still
  // reports honestly — "busy" stays exactly right when everything really is busy.
  ...
  try {
    ...
    fast = await askModel(FALLBACK_MODEL);   // a throw here reaches the caller, as it should
    usedFallback = true;
  }
  ...
  // defect that drained the account and broke his scanner mid-shift once already.
  ...
  if (read.classCode) {
    ...
    try {
      const { data: known } = codeKey
        ? await supabase.from('class_code_rental_class')
            .select('rental_class, pinned_at').eq('code', codeKey).maybeSingle()
        : { data: null };
      ...
      if (codeKey && decision.teach && decision.rentalClass) {
        await supabase.from('class_code_rental_class').upsert(
          { code: codeKey, rental_class: decision.rentalClass, learned_by: userId, updated_at: new Date().toISOString() },
          { onConflict: 'code' },
        );
      }
...
/** Is this the model being unavailable, rather than the photo being unreadable? Re-exported for
 *  the endpoint's status-code choice. */
export { isTransient };
