...
describe('buildReport — conditional sections', () => {
  ...
  // ⭐ Each fault he opened today prints with what broke (migration 150) — the auto wash, 2026-09-24.
  it('a fault prints with what broke', () => {
    const r = buildReport(baseData({
      issues: [{ reportedAt: '2026-09-24T22:02:00Z', title: 'Auto wash', severity: 'high', note: 'Rinse pipe snapped off the arch' }],
    }));
    expect(r).toContain('[High] Auto wash — Rinse pipe snapped off the arch');
  });
});
...
describe('buildReport — fuel section', () => {
  const fullFuel = {
    ...
    pump2Open: 1439, pump2Close: 1520, pump2Pumped: 81,
  ...
  it('renders all readings off the card — both metered pumps and the tank', () => {
    ...
    expect(r).toContain('1,439 → 1,520');
    expect(r).toContain('81 L pumped');
