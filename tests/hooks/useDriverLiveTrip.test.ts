...
// and (until now) untested. Written BEFORE the useReducer consolidation so the
// refactor can be proven behaviour-preserving. Locks the load-bearing transitions:
// route build → canStart, write-first start, arrive → complete, and the
// reset/abandon orphan-delete (the bug this file is known for).
...
// vi.hoisted so these exist when the (hoisted) vi.mock factories run — a factory
// that reads a value eagerly (mockResolvedValue(NON_TESLA)) would otherwise hit
// the const's temporal dead zone during the hoisted import.
...
// Recovery runs a query on mount; no-op it so tests start from a clean form.
...
function makeReady(result: { current: ReturnType<typeof useDriverLiveTrip> }) {
  act(() => result.current.setPlate('ABC123'));
...
describe('route picker', () => {
  ...
  it('handleRouteReset clears the whole route', () => {
    ...
    act(() => result.current.handleLocationTap('Airport'));
    act(() => result.current.handleLocationTap('Washbay'));
    act(() => result.current.handleRouteReset());
    expect(result.current.from).toBeNull();
...
describe('canStart', () => {
  ...
  it('an "Other" origin with no custom text blocks start', () => {
    const { result } = setup();
    act(() => result.current.setPlate('ABC123'));
    act(() => result.current.handleLocationTap('Other'));
    act(() => result.current.handleLocationTap('Washbay'));
    expect(result.current.from).toBe('Other');
    expect(result.current.canStart).toBe(false);

    act(() => result.current.setCustomFrom('Detail bay'));
...
describe('handleStart (write-first)', () => {
  it('inserts then flips to in_transit with a departure + in-progress id', async () => {
    ...
    expect(result.current.liveState).toBe('in_transit');
  ...
  it('a failed insert sets saveError and stays on the form', async () => {
    ...
    expect(result.current.liveState).toBe('form');
...
describe('handleArrived', () => {
  it('completes the in-progress trip and reports it', async () => {
    ...
    makeReady(result);
    await act(async () => { await result.current.handleStart(); });
    await act(async () => { await result.current.handleArrived(); });
    ...
    expect(result.current.liveState).toBe('complete');
    expect(result.current.submitting).toBe(false);
  });
});

// ── Reset / abandon (the orphan-bug guard) ───────────────────────────────────
...
describe('handleReset', () => {
  ...
  it('abandoning an in-transit trip deletes the orphan in-progress row', async () => {
    ...
    expect(result.current.plate).toBe('');
    expect(result.current.from).toBeNull();
    expect(result.current.inProgressId ?? null).toBeNull();
  });
});
