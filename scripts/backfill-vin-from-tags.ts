...
//
// ── WHY THIS CANNOT BE CROSS-CHECKED THE WAY THE CLASS CODE WAS ────────────────────────────────
...
// independent oracle.
//
// A VIN has no oracle. Nine characters resolve to nothing FG holds — that is precisely why it is
// worth storing, and precisely why a misread cannot be caught by comparison. So the guard is
...
//   4. COLLISIONS ARE REPORTED, NEVER RESOLVED. If two cars read the same VIN, at least one is
...
import { normalizeVinLast9 } from '../api/_lib/vinLast9.js';
...
interface Row {
  ...
  make: string; model: string; keytag_photo_url: string;
}
...
async function main(): Promise<void> {
  ...
  let rows: Row[] = await (await fetch(
    ...
    `&select=id,unit_number,license_plate,make,model,keytag_photo_url&limit=800`, { headers: H })).json();
  if (LIMIT) rows = rows.slice(0, LIMIT);

  console.log(`${rows.length} cars with a stored tag photo and no VIN${APPLY ? '' : '  (DRY RUN — writing nothing)'}\n`);

  const agree: string[] = [], disagree: string[] = [], unread: string[] = [];
