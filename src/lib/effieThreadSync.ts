...
import { packThread, unpackWrapped, type StoredMessage } from './effieThread';
...
export async function saveServerThread(messages: StoredMessage[]): Promise<void> {
  try {
    const uid = await currentUserId();
    if (!uid) return;
    const wrapped = JSON.parse(packThread(messages, Date.now())) as Wrapped;
