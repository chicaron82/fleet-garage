...
import type { Pump2Status } from './fuelReadings';
...
export function buildReport(d: ReportData): string {
  ...
  if (d.pump2Drift && d.pump2Drift !== 'ok') {
    lines.push('', 'FUEL — PUMP 2', SEP);
    lines.push(d.pump2Drift === 'used'
      ? '⚠ Pump 2 meter advanced — locked side was used this shift'
      : '⚠ Pump 2 meter dropped — possible misread or fault (cumulative meters cannot decrease)');
  }
