 ...
 */
const LARGE: readonly string[] = ['R', 'S', 'T6'];
const SUV: readonly string[] = ['B4', 'B5', 'Q4', 'L', 'L2'];
 ...
 *
 * ⚠️ AN UNKNOWN CLASS RETURNS NULL — FG says nothing rather than guessing a row. He enumerated large
 * and SUV explicitly and called the rest sedans, so "sedan" is a fallback BAND, not a list anyone
 * has verified; suggesting row 4 for a class nobody named would be inventing lot geography.
 */
export function suggestRow(rentalClass: string | null | undefined): string | null {
  const c = (rentalClass ?? '').trim().toUpperCase();
  if (!c) return null;
  if (LARGE.includes(c)) return '1';
  if (SUV.includes(c)) return '2';
  return null;
}
...
export function entryFromScan(
  ...
  holds: readonly ActiveHold[],
  carried: { status: InventoryStatus | null; row: string },
  ...
  return {
    ...
    why: d.why,
    suggestedRow: suggestRow(vehicle.rentalClass),
  };
}
