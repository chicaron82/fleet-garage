...
export function NewHoldForm({ vehicleId: preselectedId, onBack, onSuccess, onRegisterNew }: Props) {
  ...
  // Unit number search (when not pre-selected)
  ...
  const canSubmit = selectedVehicle && finalDamage && !alreadyHeld && !submitting;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    setTimeout(() => {
      addHold(selectedVehicle.id, finalDamage, notes, user!.id);
