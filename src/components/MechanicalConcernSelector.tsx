...
interface MechanicalConcernSelectorProps {
  ...
  toggleMechanicalType: (preset: string) => void;
  customMechanical: string;
  setCustomMechanical: (val: string) => void;
}

export function MechanicalConcernSelector({
  mechanicalTypes,
  toggleMechanicalType,
  customMechanical,
  setCustomMechanical,
}: MechanicalConcernSelectorProps) {
  return (
    <div>
      <div className="flex items-baseline justify-between mb-1.5">
        ...
        {mechanicalTypes.length > 0 && (
          <span className="text-xs text-blue-600 dark:text-blue-400 font-medium">
            {mechanicalTypes.length} selected
          </span>
        )}
      </div>
      ...
      {mechanicalTypes.includes('Other') && (
        <input
          ...
          value={customMechanical}
          onChange={(e) => setCustomMechanical(e.target.value)}
