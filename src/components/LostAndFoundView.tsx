        ...
        <div className="flex gap-2" onClick={e => e.stopPropagation()}>
          {item.keyTagPhotoUrl ? (
            <img
              ...
              onClick={() => { hapticLight(); onPhotoTap(item.keyTagPhotoUrl!); }}
          ...
          {item.itemPhotoUrl ? (
            <img
              src={item.itemPhotoUrl} alt="Item"
              onClick={() => { hapticLight(); onPhotoTap(item.itemPhotoUrl!); }}
              className="w-16 h-16 rounded-lg object-cover shrink-0 border border-gray-200 dark:border-gray-700 cursor-pointer active:opacity-80 transition"
            />
          ) : (
            <div className="w-16 h-16 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center shrink-0 transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center" onClick={() => setEditOpen(false)}>
          ...
          >
            <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-gray-100 dark:border-gray-800 sticky top-0 bg-white dark:bg-gray-900 z-10 transition-colors">
              <p className="text-sm font-semibold text-gray-900 dark:text-gray-100 transition-colors">Edit Item</p>
              <button type="button" onClick={() => setEditOpen(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 text-lg cursor-pointer transition">×</button>
            </div>
