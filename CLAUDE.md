...
`tests/lib/fleet-master.test.ts`). Lib coverage is strong — 103 of 111 `src/lib`
modules are tested; the remaining 8 gaps are all IO/glue (`addPersonalEvent`,
`addWhiteboardReminder`, `audit-export`, `effieThreadSync`, `hold-export`, `image`, `supabase`, `vsaTripWrite`). When a new pure function
